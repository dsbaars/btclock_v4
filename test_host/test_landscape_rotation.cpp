...
TEST_CASE("k180 maps logical (0,0) to native (nw-1, nh-1)") {
  CHECK(Eq(RotateLogicalToNative(0, 0, kNW, kNH, Rotation::k180),
           kNW - 1, kNH - 1));
}

TEST_CASE("k180 maps native bottom-right back to logical origin") {
  CHECK(Eq(RotateLogicalToNative(kNW - 1, kNH - 1, kNW, kNH, Rotation::k180),
           0, 0));
}
...
TEST_CASE("k180 is an involution on every interior pixel") {
  ...
  for (int x = 0; x < kNW; x += 7) {
    for (int y = 0; y < kNH; y += 9) {
      ...
      const auto twice = RotateLogicalToNative(once.x, once.y, kNW, kNH,
                                               Rotation::k180);
      CHECK(twice.x == x);
      CHECK(twice.y == y);
    }
...
TEST_CASE("k90Cw maps logical (0,0) to native (nw-1, 0)") {
  CHECK(Eq(RotateLogicalToNative(0, 0, kNW, kNH, Rotation::k90Cw),
           kNW - 1, 0));
}
...
TEST_CASE("k90Cw rotates the four corners cleanly") {
  ...
  CHECK(Eq(RotateLogicalToNative(0, kNW - 1, kNW, kNH, Rotation::k90Cw),
           0, 0));
}
...
TEST_CASE("k90Ccw maps logical (0,0) to native (0, nh-1)") {
  CHECK(Eq(RotateLogicalToNative(0, 0, kNW, kNH, Rotation::k90Ccw),
           0, kNH - 1));
}
...
TEST_CASE("k90Ccw is the inverse of k90Cw") {
  ...
  for (int x = 0; x < kNH; x += 11) {
    for (int y = 0; y < kNW; y += 5) {
      ...
      const auto back = RotateLogicalToNative(cw.x, cw.y, kNH, kNW,
                                              Rotation::k90Ccw);
      CHECK(back.x == x);
      CHECK(back.y == y);
    }
...
TEST_CASE("Rotation transforms preserve native bounds for interior pixels") {
  ...
  for (int rot_idx = 0; rot_idx < 4; ++rot_idx) {
    ...
    const int lw =
        (rot == Rotation::k0 || rot == Rotation::k180) ? kNW : kNH;
    const int lh =
        (rot == Rotation::k0 || rot == Rotation::k180) ? kNH : kNW;
