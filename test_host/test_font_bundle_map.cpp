...
TEST_CASE("ResolveBundleSlots maps Azeret to (Regular, Regular)") {
  // Azeret ships only the Regular cut on v4 — the SemiBold variant was
  // retired to recover Rev A flash headroom. The bold slot doubles the
  // regular so a family-aware markdown path still resolves Bundle().bold
  // to a real Font*, matching how base Antonio handles its single cut.
  const auto s = ResolveBundleSlots(FontFamily::kAzeret);
  CHECK(s.regular == FontSlot::kAzeretRegular);
  CHECK(s.bold == FontSlot::kAzeretRegular);
}
...
TEST_CASE("ResolveBundleSlots covers every FontFamily enumerator") {
  ...
  // FontFamily::kAzeret is currently the highest live id; id 23
  // (kAzeretSemiBold) was retired but the slot is intentionally not
  // reused, so the dispatch's default-Atkinson tail covers any gap.
  for (uint8_t i = static_cast<uint8_t>(FontFamily::kAntonio);
       i <= static_cast<uint8_t>(FontFamily::kAzeret); ++i) {
    const auto f = static_cast<FontFamily>(i);
    const auto s = ResolveBundleSlots(f);
    CHECK(static_cast<uint8_t>(s.regular) <=
          static_cast<uint8_t>(FontSlot::kAzeretRegular));
