...
TEST_CASE("screen kinds do not drift from ScreenType enum") {
  ...
  // 15: 8 base kinds + MiningPoolHashrate/MiningPoolEarnings/
  // MiningPoolEstimatedEarnings + BitaxeHashrate/BitaxeBestDiff +
  // NwcBalance + BlockHeightSplit. kCustom, kDebug, kNostrZap and
  // kNwcPaymentNotify are intentionally excluded — they're off-rotation
  // overrides, not user-rotatable catalogue entries.
  constexpr std::size_t kEnumCount = 15;
