...
build clean on ESP-IDF v6.0.2 and both wired boards (Rev A + Rev B) have
...
| Mining pool stats (11 pools behind shared base + TLS gate) | [mining_pool/*](https://git.btclock.dev/btclock/btclock_v3/src/commit/eac3a28/src/lib/data_sources/mining_pool) | [mining_pool_common/](../components/mining_pool_common) + per-pool components (braiins / ocean / noderunners / satoshi_radio / public_pool / blitzpool / ckpool+EU / nerdminer / viabtc / foundry) + [io/mining_pool_selector.cpp](../main/io/mining_pool_selector.cpp) reading `miningPoolName` pref; cadence configurable via `miningPoolPollMin`. v3's `gobrrr_pool` plugin was dropped in 2026-05 after upstream migrated behind a per-instance API-token wall. | Implemented | —, `-zxk`, `-zs3`, `-qmh`, `-g7h`, `-dhs`, `-9c1` |
