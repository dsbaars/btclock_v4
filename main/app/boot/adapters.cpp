...
void FrontlightAdapter::On() { fl_->On(); }
void FrontlightAdapter::Off() { fl_->Off(); }
void FrontlightAdapter::Flash() { fl_->Flash(); }
...
void LedsAdapter::SetSolidColor(uint32_t rgb) { SetLedSolidColor(rgb); }
void LedsAdapter::SetPixels(const uint32_t* rgb_array, uint32_t count) {
  SetLedPixels(rgb_array, count);
}
void LedsAdapter::SetDisabled(bool disabled) { SetLedDisabled(disabled); }
void LedsAdapter::SetBrightness(uint8_t value) { SetLedBrightness(value); }
...
bool TimerAdapter::IsPaused() const { return sm.IsPaused(); }
void TimerAdapter::SetPaused(bool paused) { sm.SetPaused(paused); }
void TimerAdapter::Restart() { sm.RestartTimer(now()); }
