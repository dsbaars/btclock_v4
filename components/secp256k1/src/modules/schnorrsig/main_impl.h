...
/* Upstream poked a precomputed SHA256("BIP0340/nonce")||SHA256("BIP0340/nonce")
 * midstate directly into sha->s/sha->bytes here to save the cost of the
 * tag-prefix block on every nonce derivation. The btclock_v4 local patch
 * routes secp256k1_sha256_* through the HW SHA peripheral via mbedtls,
 * which doesn't expose its internal state fields — so we fall back to
 * computing the tagged-init the long way. Costs ~64 SHA rounds per
 * nonce (Schnorr signing is rare here so the perf hit is negligible). */
static void secp256k1_nonce_function_bip340_sha256_tagged(secp256k1_sha256 *sha) {
    static const unsigned char kTag[] = {'B','I','P','0','3','4','0','/','n','o','n','c','e'};
    secp256k1_sha256_initialize_tagged(sha, kTag, sizeof(kTag));
}

/* See secp256k1_nonce_function_bip340_sha256_tagged above for why we
 * compute the tag the long way instead of poking a precomputed
 * SHA256("BIP0340/aux")||SHA256("BIP0340/aux") midstate. */
static void secp256k1_nonce_function_bip340_sha256_tagged_aux(secp256k1_sha256 *sha) {
    static const unsigned char kTag[] = {'B','I','P','0','3','4','0','/','a','u','x'};
    secp256k1_sha256_initialize_tagged(sha, kTag, sizeof(kTag));
}
...
/* See secp256k1_nonce_function_bip340_sha256_tagged above for why we
 * compute the tag the long way instead of poking a precomputed
 * SHA256("BIP0340/challenge")||SHA256("BIP0340/challenge") midstate. */
static void secp256k1_schnorrsig_sha256_tagged(secp256k1_sha256 *sha) {
    static const unsigned char kTag[] = {'B','I','P','0','3','4','0','/','c','h','a','l','l','e','n','g','e'};
    secp256k1_sha256_initialize_tagged(sha, kTag, sizeof(kTag));
}
