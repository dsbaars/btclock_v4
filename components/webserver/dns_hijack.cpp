...
void DnsHijack::Run() {
  ...
  while (!stop_) {
    ...
    while (p < n && buf[p] != 0) {
      if (buf[p] & 0xC0)
        break;  // compressed label — shouldn't happen in a query
      p += 1 + buf[p];
    }
    if (p >= n) continue;
    ...
    // Answer: name pointer to offset 12, TYPE A, CLASS IN, TTL 60, RDLENGTH 4,
    // RDATA
    buf[p++] = 0xC0;
    buf[p++] = 0x0C;  // pointer to QNAME
    buf[p++] = 0x00;
    buf[p++] = 0x01;  // TYPE A
    buf[p++] = 0x00;
    buf[p++] = 0x01;  // CLASS IN
